...
- `.trellis/`: workflow, task artifacts, specs, scripts, and runtime state.
...
2. **Persistence layer**: `.trellis/tasks/` and `.trellis/spec/` store task artifacts and engineering conventions.
...
| `.trellis/config.yaml` | Project configuration, task lifecycle hooks, monorepo package configuration, and task archive configuration. |
...
- To understand raw cross-session recall, read `session-memory.md`.
