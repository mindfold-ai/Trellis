...
│  Trellis                                                    │
│                                                             │
│  Feature 目录  ──> Dispatch + Ralph Loop ──> 符合规范的代码  │
│  (期望状态)         (调谐循环)                 (实际状态)     │
└─────────────────────────────────────────────────────────────┘
```
...
|------------|---------|------|
| YAML Manifest | Feature 目录 | 声明期望状态 |
...
- 可追溯 —— jsonl 记录了每个 feature 用了哪些上下文
...
| 记录 | 内容 |
|------|------|
| jsonl | 每个 feature 用了哪些上下文 |
| agent-traces | 每次 session 的工作内容 |
| feature.json | feature 的完整生命周期 |
...
| 概念 | K8s | Trellis |
|------|-----|---------|
| 期望状态 | YAML Manifest | Feature 目录 (prd.md + jsonl) |
