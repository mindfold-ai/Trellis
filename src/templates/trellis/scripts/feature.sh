...
Usage:
  $0 create "<title>" [--slug <name>] [--assignee <dev>] [--priority P0|P1|P2|P3]
...
Examples:
  ...
  $0 create "修复登录问题" --slug fix-login-bug --priority P1
  $0 init-context .trellis/agent-traces/john/features/13-add-login backend
  $0 add-context <dir> implement .trellis/structure/backend/auth.md "Auth guidelines"
  $0 set-branch <dir> feature/add-login
  $0 start .trellis/agent-traces/john/features/13-add-login
