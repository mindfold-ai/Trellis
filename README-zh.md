...
- `feature.json` 的 `backlog_ref` 指向 backlog 文件名
...
│   └── feature-utils.sh         # Feature 公共工具（查找、归档、路径安全）
...
feature.sh archive <name>                   # 归档 Feature（同时完成关联的 backlog）
feature.sh list                             # 列出活跃 Feature
feature.sh list-archive [YYYY-MM]           # 列出归档 Feature
```
