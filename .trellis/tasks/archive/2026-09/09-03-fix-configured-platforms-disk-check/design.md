...
在 `hasTrackedTemplate` 为真之外，叠加一个条件：**`configDir` 在磁盘上存在**。
...
3. **有先例**：同一函数内的 legacy Windsurf→devin 分支就是"hash + 磁盘（readdir trellis-*）"双条件。
...
- 嵌套 `configDir`（kiro `.kiro/skills`、snow `.snow/skills`）：`existsSync(join(cwd, configDir))` 对嵌套路径同样有效；父目录在但 `skills/` 被删 → 判未配置 → 重装，符合预期。
- `extraManagedPaths` / 共享 `.agents/skills`：检测键本来就不含它们（#501 已排除），本次不碰。
- manifest 缺失/损坏：`loadHashes` 返回空 → 本来就是空集，`existsSync` 短路在 `hasTrackedTemplate` 之后，无新增 I/O 失败面（`existsSync` 不抛异常）。
...
2. 现有用例即回归网：`detects every platform from the files Trellis tracked`（存在+hash → 检测到）、`does not treat native platform directories`（存在+无 hash → 不检测）保持全绿。
