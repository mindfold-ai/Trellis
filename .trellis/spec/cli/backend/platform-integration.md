...
| Per-agent-invocation context       | `before_agent_start.systemPrompt` preserves the existing full task context (PRD + jsonl) plus startup/workflow/session context; `before_agent_start.message` adds only the compact workflow/session context formerly appended by `input`                              |
...
| `before_agent_start`    | `pi.on?.("before_agent_start", …)`                   | every agent invocation (main + sub-agents) | preserves the existing `systemPrompt` return with startup/full task/workflow/session context, and additionally returns `message: { customType: "trellis-runtime-context", display: false, content }` containing compact `<workflow-state>` + `<session-overview>`; Pi wraps the message as a persisted `role: "custom"` session message |
...
const onBeforeAgentStart = (event, ctx) => {
  ...
  return {
    message: runtimeContext
      ...
      : undefined,
    systemPrompt: [cur, startup, taskContext, turn.wf, turn.ov]
      .filter(Boolean)
      .join("\n\n"),
  };
};
